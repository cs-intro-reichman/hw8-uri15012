...
public class Network {
    ...
    public String recommendWhoToFollow(String name) {
        ...
        if (user == null){ // if the name is null
            return null;
        }
        String recommendation = null;
        int maxMutual = -1; // no mutual connections have been found so far because 0 is 0 connections
        for (int i = 0; i < userCount; i++) {
            User userPotential = users [i];
            if( !user.follows(userPotential.getName()) && !userPotential.getName().equals(name)){
                int mutual = user.countMutual(userPotential);
                if (mutual > maxMutual){
                    maxMutual = mutual;
                    recommendation = userPotential.getName();
